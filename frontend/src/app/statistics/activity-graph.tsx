...
import {Calendar, GitCommit, Star, TrendingUp, BookOpen, Tag, Clock} from "lucide-react"
...
export default function BlogActivityGraph({
    ...
    const getLevelColor = (level: number) => {
        if (level === 0) return isDark ? "#1f2937" : "#f3f4f6"

        // 使用正常顺序的颜色数组，级别越高颜色越深
    ...
    const getLevelShadow = (level: number) => {
        ...
        // 使用正常顺序的颜色数组，级别越高颜色越深
        return isDark ? `0 0 8px ${selectedColors[level - 1]}50` : "none"
    }
    ...
    return (
        <div
            className={`w-full max-w-4xl rounded-xl p-6 border shadow-xl relative overflow-hidden transition-colors duration-300 ${
                isDark
                    ? "bg-gradient-to-br from-gray-900 to-gray-950 border-gray-800 text-white"
        ...
        >
            ...
            <div
                className={`absolute top-0 right-0 w-64 h-64 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 ${
                    isDark
                        ? "bg-gradient-to-br from-gray-800/10 to-transparent"
            ...
            <div className="relative z-10">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className={`text-sm ${isDark ? "text-gray-400" : "text-gray-500"}`}>每一天的更新动态捏</p>
                    </div>
                    <div
                        className={`px-4 py-2 rounded-lg border ${
                            isDark
                                ? "bg-gray-800/50 backdrop-blur-sm border-gray-700/50"
                                : "bg-white/70 backdrop-blur-sm border-gray-200/70"
                        }`}
                    >
            <span
                className={`text-2xl font-bold ${
                    isDark ? "bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent" : "text-gray-900"
                }`}
            >
              {totalContributions}
            </span>
                        <span className={`text-sm ml-2 ${isDark ? "text-gray-400" : "text-gray-500"}`}>更新</span>
                    </div>
                </div>
                ...
                {availableYears.length > 0 && (
                    <div className="mb-4">
                        <Tabs value={selectedYear} onValueChange={setSelectedYear} className="w-full">
                            <TabsList
                                className={`${
                                    isDark ? "bg-gray-800/50 border-gray-700/50" : "bg-white/70 border-gray-200/70"
                            ...
                            >
                                {availableYears.map((year) => (
                                    <TabsTrigger
                                        key={year}
                                        value={year}
                                        className={isDark ? getColorClass("bgActive") : getColorClass("bgActive")}
                                    >
                                        {year}年
                                    </TabsTrigger>
                                ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div
                        className={`rounded-lg p-3 border flex items-center ${
                            isDark
                                ? "bg-gray-800/30 backdrop-blur-sm border-gray-700/50"
                                : "bg-white/70 backdrop-blur-sm border-gray-200/70"
                        }`}
                    >
                        <div className={`p-2 rounded-full mr-3 ${isDark ? "bg-gray-700/50" : "bg-gray-100"}`}>
                            <TrendingUp className={`w-5 h-5 ${isDark ? "text-green-400" : "text-green-600"}`}/>
                        </div>
                        <div>
                            <p className={`text-xs ${isDark ? "text-gray-400" : "text-gray-500"}`}>最长连续更新</p>
                    ...
                    >
                        <div className={`p-2 rounded-full mr-3 ${isDark ? "bg-gray-700/50" : "bg-gray-100"}`}>
                            <GitCommit className={`w-5 h-5 ${isDark ? "text-blue-400" : "text-blue-600"}`}/>
                        </div>
                        <div>
                            <p className={`text-xs ${isDark ? "text-gray-400" : "text-gray-500"}`}>当前连续更新</p>
                    ...
                    >
                        ...
                        <div>
                            <p className={`text-xs ${isDark ? "text-gray-400" : "text-gray-500"}`}>单日最多更新</p>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {Object.entries(categoryStats).map(([category, count]) => (
                                <Badge
                                    ...
                                    className={`${isDark ? "border-gray-700 bg-gray-800/50" : "bg-gray-100"}`}
                                >
                                    {category}: {count}篇
                                </Badge>
                            ))}
            ...
            {/* 热力图 */}
            <div
                className={`rounded-xl p-4 border relative z-10 ${
                    isDark
                        ? "bg-gray-800/20 backdrop-blur-sm border-gray-700/50"
                        : "bg-white/70 backdrop-blur-sm border-gray-200/70"
                }`}
            >
                {isLoading ? (
                    <div className="flex flex-col items-center justify-center py-12">
                        <div
                            className={`w-10 h-10 border-4 rounded-full animate-spin mb-4 ${
                                isDark ? "border-gray-600 border-t-gray-200" : "border-gray-300 border-t-gray-600"
                            }`}
                        ></div>
                            ...
                            <div className="w-full">
                                <div className="flex mb-1 relative">
                                    {monthLabels.map((month, index) => (
                                        <span
                                            key={index}
                                            className={`text-xs absolute ${isDark ? "text-gray-400" : "text-gray-500"}`}
                                            ...
                                            <TooltipProvider key={`${weekIndex}-${dayIndex}`}>
                                                <Tooltip>
                                                    ...
                                                    {day && (
                                                        <TooltipContent
                                                            side="top"
                                                            className={`p-3 rounded-lg shadow-lg z-50 max-w-[280px] backdrop-blur-sm ${
                                                                isDark
                                                        ...
                                                        >
                                                            <div className="text-sm">
                                                                ...
                                                                <div
                                                                    className={`font-medium ${isDark ? getColorClass("text") : getColorClass("text")}`}
                                                                >
                                                                    {day.count} 篇更新
                                                                </div>
                                                                {day.posts.length > 0 && (
                                                                    <div
                                                                        ...
                                                                        {day.posts.map((post, i) => (
                                                                            <div
                                                                                key={i}
                                                                                className={`mb-2 p-2 rounded-md text-xs transition-colors ${
                                                                                    isDark
                ...
                <div className="flex justify-end items-center mt-4">
                    <span className={`text-xs mr-2 ${isDark ? "text-gray-500" : "text-gray-400"}`}>更少</span>
                    <div className="flex gap-1">
                        <div
                            className={`w-3 h-3 rounded-sm ${
                                isDark ? "bg-gray-800 border border-gray-700/50" : "bg-gray-100 border border-gray-200"
                            }`}
                        />
                    ...
                    <span className={`text-xs ml-2 ${isDark ? "text-gray-500" : "text-gray-400"}`}>更多</span>
                </div>
            </div>
            ...
            <AnimatePresence>
                {selectedPost && (
                    <motion.div
                        className={`mt-6 rounded-xl p-4 border relative z-10 ${
                            isDark
                                ? "bg-gray-800/30 backdrop-blur-sm border-gray-700/50"
                                : "bg-white/70 backdrop-blur-sm border-gray-200/70"
                        }`}
                        ...
                        <div className="flex items-center gap-2 mt-4">
                            ...
                            >
                                <Button
                                    ...
                                    className={
                                        isDark
                                            ? getColorClass("bg") + " " + getColorClass("bgHover") + " text-white"
                                            : getColorClass("bg") + " " + getColorClass("bgHover") + " text-white"
                                    }
                                >
            ...
            {/* 底部信息 */}
            <div className={`mt-4 text-xs text-center ${isDark ? "text-gray-500" : "text-gray-400"}`}>
