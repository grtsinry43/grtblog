...
::: tip 查看可用版本
- GitHub Releases: `https://github.com/grtsinry43/grtblog/releases`
- Docker Hub: `https://hub.docker.com/r/grtsinry43/grtblog-server/tags`
- GHCR: `https://github.com/grtsinry43/grtblog/pkgs/container/grtblog-server`
- CNB: `https://cnb.cool/grtsinry43/grtblog`
:::

### 3. 启动服务

```bash
mkdir -p storage/html storage/meta/isr storage/uploads storage/geoip
docker compose up -d
```
...
**推荐直接重跑一键安装脚本**，它会自动进入升级模式：备份并同步 `docker-compose.yml` 和 `nginx/nginx.conf`、更新 `.env` 中的版本号、拉取新镜像、重启服务并重载 nginx 配置，你的密码密钥等配置保持不变：
...
如果选择手动升级，注意 **`docker-compose.yml` 和 `nginx/nginx.conf` 也是版本的一部分**，新版本可能调整了卷挂载、路由或缓存策略，只换镜像不换配置可能导致新功能不生效甚至启动失败：
...
# 3. 拉取新镜像并重启（compose 会自动重建定义有变化的容器）
docker compose pull
docker compose up -d

# 4. 重载 nginx 配置
# nginx.conf 是挂载进容器的，up -d 不会因文件内容变化而重建容器，必须显式 reload
docker compose exec nginx nginx -t && docker compose exec nginx nginx -s reload
```

数据库迁移和静态页面全量重渲在 server 启动时自动执行。

::: warning nginx.conf 请勿手动修改
`nginx/nginx.conf` 由 grtblog 管理，每次升级会被覆盖（脚本会自动备份旧文件）。自定义反代规则请放在外层反向代理中，参考下文「反向代理与 HTTPS」。
:::
...
```bash
# 拉取最新代码并切换到目标版本（docker-compose.yml 与 nginx.conf 会随代码一起更新）
git fetch --tags
git checkout v2.1.0
...
# 重载 nginx 配置（nginx.conf 是挂载文件，up -d 不会触发重载）
docker compose exec nginx nginx -t && docker compose exec nginx nginx -s reload
```
...
| `./storage/meta` | ISR 版本标记与资源清单（可随时重新生成） |
