<script lang="ts">
	import { Github, Mail, MessageSquare, History, User, Info, Archive, Link as LinkIcon, Rss, BarChart3, Activity } from 'lucide-svelte';
...
<footer class="mt-32 border-t border-jade-100/80 dark:border-ink-800 bg-jade-50/30 dark:bg-ink-950/30 backdrop-blur-sm">
	<div class="max-w-[1200px] mx-auto px-6 py-12 md:py-16">
		...
		<div class="flex flex-col gap-4 mb-12 md:hidden">
			{#each footerSections as section}
				<div class="flex flex-col gap-2">
					<div class="text-sm font-serif font-bold text-ink-900 dark:text-ink-100 flex items-center justify-between">
						{section.title}
						<span class="text-ink-300 dark:text-ink-700 font-mono font-normal">{">"}</span>
					</div>
					<div class="flex flex-wrap gap-x-4 gap-y-2">
						{#each section.links as link}
							<a href={link.href} class="text-sm text-ink-500 hover:text-jade-600 dark:hover:text-jade-400 transition-colors">
								{link.name}
							</a>
						{/each}
					</div>
		...
		<div class="hidden md:grid grid-cols-4 gap-12 mb-16">
			{#each footerSections as section}
				<div class="flex flex-col gap-6">
					<h3 class="text-sm font-serif font-bold text-ink-900 dark:text-ink-100 flex items-center gap-2">
			...
			<div class="flex flex-col gap-6 items-end text-right">
				<div class="flex flex-col items-end">
					...
					<p class="text-[11px] font-mono text-ink-400 mt-1 uppercase tracking-wider">A blog framework for developers</p>
				</div>
				<div class="flex items-center gap-2 px-2.5 py-1 rounded-full bg-jade-500/5 border border-jade-500/10 w-fit">
					<span class="relative flex h-1.5 w-1.5">
						<span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-jade-400 opacity-75"></span>
		...
		<div class="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-ink-100 dark:border-ink-800/50 gap-4">
			<div class="text-[10px] md:text-[11px] font-mono text-ink-400 text-center md:text-left">
				...
				<div class="flex flex-wrap justify-center md:justify-start gap-x-3 mt-1">
					<a href="https://beian.miit.gov.cn/" target="_blank" rel="noreferrer" class="hover:text-jade-600 transition-colors">湘ICP备2023033970号-1</a>
