...
<div class="space-y-6">
	<div class="flex items-center justify-between border-b border-ink-50 pb-2 dark:border-ink-800/30">
		...
		<a href="/moments" class="group text-[10px] text-ink-300 transition-colors hover:text-jade-500">
			<ArrowRight size={10} class="transition-transform group-hover:translate-x-0.5" />
		</a>
	</div>

	{#if $relatedMomentsStore.length === 0}
		<div class="rounded-default border border-dashed border-ink-100 bg-ink-50/30 p-3 text-[10px] text-ink-400 dark:border-ink-800/40 dark:bg-ink-900/20 dark:text-ink-500">
			暂无同期手记
		</div>
	{:else}
		<div class="space-y-4">
			{#each $relatedMomentsStore as moment, i (moment.id)}
				...
				>
					...
					<h4 class="text-[11px] font-bold leading-snug text-ink-800 transition-colors group-hover:text-jade-600 dark:text-ink-200">
						{moment.title}
					</h4>
