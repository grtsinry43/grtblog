<script lang="ts">
	...
	const options = [
		{ id: 'posts', name: '文章', desc: '深度思考与技术分享', icon: Newspaper, color: 'text-jade-500' },
		{ id: 'moments', name: '手记', desc: '生活碎片与即时感悟', icon: Coffee, color: 'text-amber-500' },
		{ id: 'thinkings', name: '思考', desc: '碎片化的逻辑与灵感', icon: Brain, color: 'text-purple-500' },
...
{#if isOpen}
	...
	>
		<div class="overflow-hidden rounded-default border border-ink-200/60 bg-white shadow-deep dark:border-ink-800 dark:bg-ink-900 noise-surface">
			<!-- Header -->
			<div class="relative p-8 pb-4">
				...
				<p class="text-sm text-ink-500">选择你感兴趣的内容，当有新产出时，我会第一时间发邮件通知你。</p>
			</div>
			...
			{#if !isSuccess}
				<form onsubmit={handleSubmit} class="p-8 pt-4">
					<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
						{#each options as opt}
							...
							>
								...
								<div>
									<div class="text-sm font-medium {preferences[opt.id as keyof typeof preferences] ? 'text-ink-900 dark:text-ink-100' : 'text-ink-500'}">{opt.name}</div>
					...
					<div class="relative group">
						<Mail size={18} class="absolute left-4 top-1/2 -translate-y-1/2 text-ink-400 group-focus-within:text-jade-500 transition-colors" />
						<input 
							type="email" 
							bind:value={email}
							required
			...
			{:else}
				<div class="p-12 text-center" in:fly={{ y: 10 }}>
					<div class="w-16 h-16 bg-jade-100 dark:bg-jade-900/30 text-jade-600 rounded-full flex items-center justify-center mx-auto mb-6">
						<Check size={32} />
					</div>
