<script lang="ts">
	import type { TimelineStats, UnifiedTimelineItem } from '../types';
	...
	function scrollTimelineByDelta(delta: number) {
		if (!containerRef || delta === 0) return;
		const nextScrollY = clamp(
			scrollY + delta * SCROLL_RATIO,
	...
	const canConsumeTimelineDelta = (delta: number) => {
		if (!containerRef || timelineScrollRange <= 0) return false;
		const start = containerRef.offsetTop;
		const end = start + timelineScrollRange;
		return delta > 0 ? scrollY < end - 0.5 : scrollY > start + 0.5;
	};
	...
	<div bind:this={containerRef} class="relative w-full" style="height: {verticalHeight}px;">
		<div
			use:timelineGesture={{
				...
				canConsume: canConsumeTimelineDelta,
				onDelta: scrollTimelineByDelta
			}}
			...
			<div class="relative h-full w-full">
				...
				>
					...
					{#each visibleItems as item (item.id)}
						...
						{@const currentY = prefersReducedMotion.current
							? item.targetY!
							: isPast
								? item.targetY!
								: item.targetY! * flyProgress}
						{@const currentXOffset = prefersReducedMotion.current
							? 0
							: isPast
								? 0
								: (1 - flyProgress) * 400}
						...
						>
							...
							<div
								...
								transform:
									...
									translate({currentXOffset}px, {prefersReducedMotion.current || isPast
									? 0
									: (1 - flyProgress) * 300}px)
									translateY({currentY}px)
									scale({prefersReducedMotion.current ? 1 : 0.9 + focusWeight * 0.1})
									rotate({prefersReducedMotion.current || isPast ? 0 : (1 - flyProgress) * 10}deg)
