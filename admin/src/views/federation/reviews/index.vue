...
import { getFederationPendingReviews, reviewFederationCitation, reviewFederationMention } from '@/services/federation-admin'
...
const columns: DataTableColumns<FederationReviewItemResp> = [
  ...
  { title: '申请时间', key: 'requested_at', width: 180, render: (row) => new Date(row.requested_at).toLocaleString() },
  {
    title: '操作',
    key: 'actions',
    width: 200,
    ...
    <NCard :bordered="false">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
           <NButton secondary size="small" :loading="isPending" @click="queryClient.invalidateQueries({ queryKey: ['federation-pending-reviews'] })">
            刷新
          </NButton>
        </div>
      </div>
