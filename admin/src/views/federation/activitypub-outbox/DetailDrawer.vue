...
import { NDrawer, NDrawerContent, NDescriptions, NDescriptionsItem, NTag, NCode, NButton, NDataTable, useMessage } from 'naive-ui'
...
const canRetry = computed(() => displayItem.value?.status === 'failed' || displayItem.value?.status === 'partial')
...
const columns: DataTableColumns<ActivityPubDeliveryDetailResp> = [
  ...
  {
    title: '投递时间', key: 'delivered_at', width: 170,
    render(row) {
...
<template>
  <NDrawer :show="show" :width="drawerWidth" @update:show="(v) => emit('update:show', v)">
    <NDrawerContent title="ActivityPub 出站详情" closable>
      <div v-if="displayItem" class="space-y-6">
        <NDescriptions bordered :column="2" label-placement="left" title="基本信息">
          <NDescriptionsItem label="ID">{{ displayItem.id }}</NDescriptionsItem>
          <NDescriptionsItem label="状态"><NTag :type="statusTagType(displayItem.status)" size="small">{{ displayItem.status }}</NTag></NDescriptionsItem>
          <NDescriptionsItem label="SourceType">{{ displayItem.source_type }}</NDescriptionsItem>
          <NDescriptionsItem label="TriggerSource">{{ displayItem.trigger_source }}</NDescriptionsItem>
          ...
          <NDescriptionsItem label="统计">{{ displayItem.success_count }}/{{ displayItem.total_targets }}</NDescriptionsItem>
          <NDescriptionsItem label="耗时(ms)">{{ displayItem.duration_ms ?? '-' }}</NDescriptionsItem>
          <NDescriptionsItem label="开始时间">{{ displayItem.started_at ? new Date(displayItem.started_at).toLocaleString() : '-' }}</NDescriptionsItem>
          <NDescriptionsItem label="结束时间">{{ displayItem.finished_at ? new Date(displayItem.finished_at).toLocaleString() : '-' }}</NDescriptionsItem>
        </NDescriptions>

        <div>
          <div class="mb-2 flex items-center justify-between">
            <h3 class="font-medium">投递明细</h3>
            <NButton v-if="canRetry" type="warning" :loading="retrying" @click="handleRetry">重试失败投递</NButton>
          </div>
          <NDataTable :columns="columns" :data="displayItem.deliveries || []" :loading="loading" :scroll-x="1000" />
        </div>

        <div>
          <h3 class="mb-2 font-medium">Activity JSON</h3>
          <NCode :code="activityCode" language="json" word-wrap />
        </div>
      </div>
      <template #footer>
