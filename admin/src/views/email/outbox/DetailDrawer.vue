...
import { NDrawer, NDrawerContent, NDescriptions, NDescriptionsItem, NTag, NCode, NButton, NTabs, NTabPane } from 'naive-ui'
...
<template>
  <NDrawer :show="show" :width="drawerWidth" @update:show="(val) => emit('update:show', val)">
    <NDrawerContent title="邮件出站详情" closable>
      <div v-if="displayItem" class="space-y-6">
        <NDescriptions bordered :column="1" label-placement="left" title="基本信息">
          ...
          <NDescriptionsItem label="事件名">
            <NTag type="info" size="small">{{ displayItem.eventName }}</NTag>
          </NDescriptionsItem>
          <NDescriptionsItem label="收件人">{{ displayItem.toEmails?.join(', ') || '-' }}</NDescriptionsItem>
          ...
          <NDescriptionsItem label="发送时间">{{ displayItem.sentAt ? new Date(displayItem.sentAt).toLocaleString() : '-' }}</NDescriptionsItem>
        </NDescriptions>

        <NDescriptions bordered :column="1" label-placement="left" title="重试信息">
          <NDescriptionsItem label="重试次数">{{ displayItem.retryCount }}</NDescriptionsItem>
          <NDescriptionsItem label="下次重试">{{ displayItem.nextRetryAt ? new Date(displayItem.nextRetryAt).toLocaleString() : '-' }}</NDescriptionsItem>
        </NDescriptions>

        <div v-if="displayItem.lastError">
          <h3 class="mb-2 font-bold">错误信息</h3>
          <NCode :code="displayItem.lastError" language="text" word-wrap />
        </div>

        <div v-if="displayItem.htmlBody || displayItem.textBody">
          <NTabs type="line" size="small">
            <NTabPane v-if="displayItem.htmlBody" name="html" tab="HTML 正文">
              <div class="max-h-80 overflow-auto rounded border p-2" v-html="displayItem.htmlBody" />
            </NTabPane>
            <NTabPane v-if="displayItem.textBody" name="text" tab="纯文本">
              <NCode :code="displayItem.textBody" language="text" word-wrap />
            </NTabPane>
          </NTabs>
        </div>
      </div>
