...
<template>
  <ScrollContainer wrapper-class="p-4 md:p-6 space-y-4">
    <PageHeader title="高级信息 / Observability" icon="ph--desktop" class="mb-4">
      <template #badge>
        ...
        <template v-if="!loading || item.value">
          <div class="flex-1">
            ...
            <div class="flex items-center">
              <span class="text-xs text-neutral-500 dark:text-neutral-400">{{ item.description }}</span>
            </div>
          </div>
          <div>
    ...
    <div class="grid grid-cols-1 gap-4 overflow-hidden max-sm:gap-2 lg:grid-cols-12">
      ...
      <div class="col-span-1 lg:col-span-6">
        <div class="flex flex-col rounded border border-naive-border bg-naive-card p-5 transition-[background-color,border-color]">
          <div class="mb-4 text-base font-medium text-neutral-600 dark:text-neutral-300">控制平面概况</div>
      ...
      <div class="col-span-1 lg:col-span-6">
        <div class="flex flex-col rounded border border-naive-border bg-naive-card p-5 transition-[background-color,border-color]">
          <div class="mb-4 text-base font-medium text-neutral-600 dark:text-neutral-300">实时与存储</div>
          <NDescriptions :column="2" size="small">
            ...
            <NDescriptionsItem label="WS Fanout P95">{{ realtimeData?.snapshot.broadcastP95Ms?.toFixed(1) }} ms</NDescriptionsItem>
    ...
    <SystemHealthCard :component-healths="componentHealths" :component-tag-type="componentTagType" />
    ...
    <div class="rounded border border-naive-border bg-naive-card p-5 transition-[background-color,border-color]">
      <div class="mb-4 flex items-center justify-between">
        ...
        <NTag size="small" :type="updateTagType() as any" round>
          {{ updateInfo?.status === 'error' ? '检查失败' : updateInfo?.status === 'disabled' ? '已关闭' : updateInfo?.hasUpdate ? '可更新' : '已最新' }}
        </NTag>
      </div>
      <NEmpty v-if="!updateInfo" description="暂无更新信息" />
      <NDescriptions v-else :column="2" size="small">
        ...
        <NDescriptionsItem label="目标版本">
          ...
          <NTag v-if="updateInfo.targetRelease?.prerelease" class="ml-2" size="tiny" type="warning" round>测试版</NTag>
        </NDescriptionsItem>
