...
defineProps<{
  componentHealths: Array<{ name: string; healthy: boolean; status: string; version?: string; checkedAt?: string }>
...
<template>
  <div class="rounded border border-naive-border bg-naive-card p-5 transition-[background-color,border-color]">
    ...
    <NEmpty v-if="!componentHealths.length" description="暂无组件健康数据" />
