...
export interface UpgradeGuideVersion {
  /** Stable task id, independent from the release version. */
  id: string
...
export function getPendingGuides(pendingTasks: Array<{ id: string }>): UpgradeGuideVersion[] {
  const ids = new Set(pendingTasks.map((task) => task.id))
  return upgradeGuideRegistry.filter((guide) => ids.has(guide.id))
}
